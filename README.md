...
| Path | What it is |
|---|---|
| `deck/` | The course slide deck (71 slides, no speaker notes) — theory for all modules: architecture & sizing, ingestion, indexing & compression, querying, replication & failure, observability, security/RBAC, multi-tenancy, backup & upgrade |
| `course/` | Participant workbook (the Lab 1 sizing worksheet and course reference sheets) |
| `labs/` | Four hands-on labs + capstone: sizing (paper), benchmark & tune (`acme` harness), failure & recovery, observability & incident — each with checkpoints and fallback packs |
| `on-premise/` | Self-study module for teams running self-hosted Weaviate: operations guide + intense, symptom-driven troubleshooting playbooks |
...
---
© Weaviate — training material. Estimates in these materials are labeled estimates and are not sizing guarantees.
